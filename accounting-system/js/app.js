...
async function addBillItemRow(expenseCode = '', qty = 1, price = 0, hasVat = false, whtRate = 'none') {
    ...
    const templates = await getExpenseCatalog();
...
function bindUIActions() {
    ...
    document.getElementById('bill-wht-rate')?.addEventListener('change', recalculateBill);
    ...
    document.getElementById('inv-wht-rate')?.addEventListener('change', recalculateInvoice);
...
async function generateDocumentIds() {
    ...
    const prefixDP = `DP${yy}${mm}-`;
    const prefixVR = `VR${yy}${mm}-`;
    
    const currentMonthDPs = dps.filter(x => x.id.startsWith(prefixDP));
    let nextDPSeq = 1;
    if (currentMonthDPs.length > 0) {
        const seqs = currentMonthDPs.map(x => parseInt(x.id.replace(prefixDP, '')) || 0);
        nextDPSeq = Math.max(...seqs) + 1;
    }
    const pcPayId = document.getElementById('pc-pay-id');
    if (pcPayId) pcPayId.value = prefixDP + nextDPSeq.toString().padStart(4, '0');
    
    const currentMonthVRs = vrs.filter(x => x.id.startsWith(prefixVR));
    let nextVRSeq = 1;
    if (currentMonthVRs.length > 0) {
        const seqs = currentMonthVRs.map(x => parseInt(x.id.replace(prefixVR, '')) || 0);
        nextVRSeq = Math.max(...seqs) + 1;
    }
    const pcReimId = document.getElementById('pc-reim-id');
    if (pcReimId) pcReimId.value = prefixVR + nextVRSeq.toString().padStart(4, '0');
}
